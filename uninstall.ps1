...
# 移除注入的 index.html 内容与自定义资源/服务文件。
# 用法：
...
foreach ($f in @('background.jpg', 'custom-background.css', 'custom-background.js')) {
...
Write-Host "[2/3] 自定义资源已删除"

# 3) 删除服务与配置
foreach ($f in @('terminal-bridge.mjs', 'gitlab-bridge.mjs', 'gitlab-tasks.ps1', 'gitlab-fetch.ps1', 'metrics-writer.ps1', 'gitlab-config.json')) {
